...
class LeaveRequest(models.Model):
    ...
    def clean(self):
        ...
        # Exception: Sick Leave and Emergency Leave can be filed on the same day
        if self.start_date and self.leave_type:
            advance_days = (self.start_date - today).days
            is_exempt = self.leave_type.name.lower() in [
                'sick leave', 'sick', 'emergency leave', 'emergency'
            ]
            if not is_exempt and advance_days < 2:
                raise ValidationError(
                    f"{self.leave_type.name} must be filed at least 2 days in advance."
                )
